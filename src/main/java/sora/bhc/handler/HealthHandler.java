...
//                int extraHearts = 0;
//                for(int i = 0; i < hearts.length; i++) {
//                    extraHearts += MathHelper.clamp(hearts[i], 0, OLDCONFIG.heartStackSize * 2); //make sure to not bypass the limit; bugfix: this is half hearts, so we need to double the limit...
//                }
